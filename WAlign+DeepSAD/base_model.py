...
class BaseDeepAD(metaclass=ABCMeta):
    ...
    def fit(self, X, y=None, cluster_labels=None):
        self.train_data = X
        self.train_label = y
        ...
        for _ in range(self.n_ensemble):
            self.train_loader, self.net, self.criterion = self.training_prepare(self.train_data,
                                                                                y=self.train_label, cluster_labels=self.train_cluster_labels)
            self._training()
    ...
    def _training(self):
        ...
        for i in range(self.epochs):
            if i > 0:
                self._compute_epoch_weights(self.net)
                new_weights = torch.from_numpy(self.sample_weights).float()

                self.train_loader.dataset.tensors = (
                    self.train_loader.dataset.tensors[0], 
                    self.train_loader.dataset.tensors[1],
                    self.train_loader.dataset.tensors[2],
                    new_weights
                )
            ...
            t = time.time() - t1
            print(f'epoch {i+1:3d}, training loss: {total_loss/cnt:.6f}, time: {t:.1f}s')
