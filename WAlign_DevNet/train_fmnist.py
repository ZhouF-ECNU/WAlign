...
from datasets.fashionmnist import LeNetAutoencoder
...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        outlier_idx = np.where(np.isin(idx, [0]))[0]

        self.train_loader = DataLoader(dataset=self.train_set, worker_init_fn=worker_init_fn_seed, batch_sampler=BalancedBatchSampler(args, self.train_set, normal_1_idx, normal_2_idx, outlier_idx))
    ...
    def train(self, epoch):
        ...
        for i, sample in enumerate(self.train_loader):
            ...
            _, nearest_indices = torch.topk(distances[:n_normal*2], k=11, largest=False, dim=1)
            nearest_indices = nearest_indices[:, 1:]
            nearest_states = state[nearest_indices]
            probabilities = nearest_states.sum(dim=1).float() / 10
            ...
            corrected_weights = ((torch.min(torch.full_like(s_selected, 2), torch.abs(s_selected)) / sigma_) 
                     + ((1.0 - p_selected) / 0.5)) / 2
...
if __name__ == '__main__':
    ...
    parser.add_argument("--epochs", type=int, default=80, help="the number of epochs")
    ...
    parser.add_argument("--batch_size", type=int, default=96, help="batch size used in SGD")
    parser.add_argument("--random_seed", type=int, default=45, help="the random seed number")
    ...
    dataset = FashionMNIST_Dataset(root='./data', model=model, device=device, random_seed=args.random_seed)
    ...
    result_data = [args.epochs, args.batch_size, args.lr, args.weight_decay, round(auroc, 4), round(auprc, 4)]
    results_header = ['epochs', 'batch_size', 'lr', 'weight_decay', 'AUROC', 'AUPRC']
    update_results_file('/home/lgy/myModel/WAlign_code_github/WAlign+DevNet/results/results.csv', results_header, result_data)