...
class BalancedBatchSampler(Sampler):
    def __init__(self, cfg, dataset, normal_1_idx, normal_2_idx, normal_3_idx, outlier_idx):
        ...
        self.normal_3_generator = self.random_generator(normal_3_idx)
        self.outlier_generator = self.random_generator(outlier_idx)
        if self.cfg.n_anomaly != 0:
            self.n_normal_1 = self.cfg.batch_size // 4
            self.n_normal_2 = self.cfg.batch_size // 4
            self.n_normal_3 = self.cfg.batch_size // 4
            self.n_outlier = self.cfg.batch_size - self.n_normal_1 - self.n_normal_2 - self.n_normal_3
        else:
            self.n_normal_1 = self.cfg.batch_size // 3
            self.n_normal_2 = self.cfg.batch_size // 3
            self.n_normal_3 = self.cfg.batch_size // 3
            self.n_outlier = 0
    ...
    def __iter__(self):
        for _ in range(self.cfg.steps_per_epoch):
            ...
            for _ in range(self.n_normal_3):
                batch.append(next(self.normal_3_generator))
